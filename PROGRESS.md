...
## 2026-09-15 (4) — 사이트를 게임 포털 배치로

### 먼저 쟀다

| | 전 | 후 |
|---|---|---|
| 타일 크기 | 318×624 | **138×209** |
| 폴드 위 게임 수 (1280×800) | **3개** | **24개** |
| 히어로 높이 | 400px | 96px 띠 |
| 게임 페이지에서 게임이 차지하는 폭 | **36%** (856px 상자에 세로 게임) | **100%** |
| LCP (홈) | 800ms | **332ms** |
| CLS | 0.008 | **0.000** |

파일 크기는 문제가 아니었다 — 썸네일 24장 합계 224KB, 평균 9KB. 문제는 **표시 크기**였다.

### 무엇을 바꿨나

- **타일**: 이름 한 줄만 남기고 태그·설명 문장을 뺐다. 문장은 아래 목록 절에 그대로 있어
  읽을 것이 줄지 않는다. 데스크톱 8열, 폰 3열
- **히어로**: 400px 블록 → 96px 가로 띠. 새 게임 자리는 그대로 두되 폴드를 안 먹는다
- **게임 페이지**: 판의 비율을 **밴드(390/806) 그대로** 맞췄다. 세로 게임을 가로 상자에
  넣으면 나머지는 전부 게임이 스스로 그린 배경이다. 넓은 화면의 옆 빈자리에는 **다음 게임 4개 레일**
- **색**: 게임 24개가 전부 따뜻한 색이라 차가운 파스텔 바탕 위에서 타일이 붙어 보이지 않았다.
  따뜻한 종이 + 거의 검정 잉크 + 펠트 코랄 강조 하나. **모든 글자 쌍 4.5:1 이상 실측 확인**
- 죽은 CSS(사이드바·모달 플레이어·지갑 알약)와 `app.js` 로드를 걷어냈다

### 썸네일 — 어두운 게임이 까만 사각형이었다

격자를 138px로 줄이자 24장 중 **16장이 거의 까만 네모**가 됐다.
`space-shooter` 썸네일의 평균 밝기는 255 중 **1.3**이었다. 게임은 멀쩡한데 손톱만 하게
줄이면 아무것도 안 보인다.

`tools/shoot.js`에 **측정 기반 밝기 보정**을 넣었다. 평균이 바닥(46) 아래면 그만큼만 올리고
밝은 게임은 손대지 않는다. 내용은 바꾸지 않는다. 약한 타일 16 → **7장**.

### 그러다 내가 게임오버 화면을 썸네일로 박았다

더 많은 게 보이게 캡처 창을 뒤로 밀었더니 `neon-dodge`·`dino-jump` 썸네일이
**"RETRY / LOBBY"** 화면이 됐다. 이 게임들은 자동 테스터 손에서 1~2초 만에 죽어서
모든 표본이 죽은 뒤였다. 원래 그걸 막고 있던 것이 '가장 이른 변화 프레임' 규칙인데,
`captureFromMs` override가 그 보호를 무력화했다 — 내 노트(thumbnail-that-lied)가
설명하는 바로 그 실패를 내가 다시 했다.

**도구를 고쳤다: 멈춘 프레임을 거절한다.** 플레이 중 화면은 계속 바뀌고 게임오버 오버레이는
멈춰 있다. 후보를 잡을 때마다 260ms 뒤와 비교한다. **실패가 아니라 경고**로 둔 이유는
느린 퍼즐판도 멈춘 것으로 읽히기 때문이다 — 실제로 4개를 헛경고하고 2개를 정확히 잡았다.
처음 구현은 전부 거절해서 `cyber-snake` 캡처를 통째로 깨뜨렸다(차선책 프레임을 안 남겼다).

### 아직 남은 것 — 솔직하게

**격자가 두 사이트로 보인다.** 위 5개는 따뜻한 펠트, 아래 19개는 어두운 네온이다.
디자인으로 가릴 수 있는 문제가 아니다 — "공통점 없는 게임 19개"가 이제 한눈에 보이는 것이다.
접을지 계속할지의 판단은 이 화면을 보고 하는 게 맞다.

---

## 2026-09-15 (3) — Feltfoot: 결과물이 그림이 아니라 '움직임'인 게임

### 왜 이걸 만들었나

무승부 게임이 퍼지는 조건을 먼저 정리했다. **플레이어에게 남에게 보여줄 물건을 쥐여준다** —
Neko Atsume(고양이 사진), Infinite Craft(웃긴 조합), Wordle(공유 격자), Line Rider(내가 만든 트랙).
조건 둘: **사람마다 다를 것**, **3초 안에 읽힐 것**.

이 기준으로 **Feltling은 실격**이다. 다 키우면 모두의 물범이 픽셀 단위로 똑같다. 보여줄 게 없다.

그리고 결과물이 *그림*인 자리는 10년째 꽉 차 있다. 검색으로 확인했다 —
마블링([paint.toys](https://paint.toys/marbling/), Ghassaei WebGL), 니들 펠팅
([itch.io 브라우저 게임](https://chiaburn.itch.io/heartily-needle-felting-cat), 모바일 ASMR 앱),
타이다이·모래·눈꽃·도자기·슬라임. 내 후보 2개가 여기서 죽었다.

**비어 있는 자리는 결과물이 그림이 아닌 것이었다.** Feltfoot의 결과물은 걸음걸이다.

### 이름

`Feltfoot`. 검색으로 확인, 충돌 없음. `Toddle`은
[데일리 단어 퍼즐](https://toddle-puzzle.web.app/)과 교육 회사가 쓰고 있어 버렸다.
`Feltling`과 `Felt-` 접두를 공유하는 것이 브랜드 자산이다.

### 만든 것

몸통에 부품(다리·팔·귀·뿔·꼬리·눈·입)을 탭으로 꿰맨다 → 실을 끊으면 일어나서 걷는다 →
선반에 쌓인다. **걸음걸이가 붙인 위치에서 계산된다.** 짝짝이 다리면 절뚝, 옆구리면 비틀,
다리가 없으면 굴러간다. 아무도 애니메이션을 만들지 않았고, 결과의 가짓수가 사실상 무한이며
아트 시간이 0이다.

### 물리에서 배운 것 두 가지

**1. 몸통은 한 점이 아니라 세 점이다.** 한 점은 방향이 없어서 기울지도 넘어지지도 못한다.
기울 수 없으면 짝짝이 다리가 아무 차이도 못 만들고, 그러면 이 게임의 전제가 통째로 사라진다.
세 점이 국소 좌표계를 주고, 부품은 그 좌표계에 부착점을 기록한다.

**2. 완전 물리 걸음은 실패했다.** 발을 땅에 고정하고 몸통에 전진 속도를 더하는 방식 —
다리가 접혀 주저앉길래 무릎을 뻣뻣하게(0.96 길이, 강성 0.55) 했더니 그 힘이 인형을
화면 밖으로 **쏘아 보냈다**. 튜닝할 것이 너무 많고, 실패했을 때 "서툰 인형"이 아니라
**"고장난 프로그램"**으로 보인다 — 이 둘은 플레이어에게 완전히 다른 것이다.

채택한 것은 **몸통은 운동학, 다리는 물리**인 혼합이다. 몸통을 목표 높이로 *약하게* 당긴다
(계수 0.055). 세게 당기면 몸이 경로를 미끄러지고 다리는 장식이 되어, 움직임이 내가 만든 것과
무관해진다. 약하게 당기면 다리가 일을 해야 하고, **목표 높이에 못 닿는 다리가 매 걸음
실패하는 것이 곧 절뚝임이다.**

### 썸네일 도구가 또 설계 문제를 잡았다

첫 썸네일이 **공유 카드 모달 사진**이었다. 캡처 도구가 화면 중앙을 누르는데,
살아 있는 동안 화면을 누르면 카드가 떴기 때문이다.

나쁜 스크린샷이자 더 나쁜 설계였다. 뒤뚱거리는 걸 보고 있는 사람이 하고 싶은 일은
**찔러 보는 것**이지 스크린샷 저장이 아니다. 탭 = 쿡 찌르기(임펄스 + 먹먹한 소리)로 바꾸고
카드는 버튼으로 옮겼다. `captureQuery=show=1`로 데모 인형을 띄워 찍는다.

### 아직 모르는 것

**사람이 실제로 공유할지는 검증되지 않았다.** 이 게임의 전제는
"움직임 때문에 남에게 보여준다" 하나이고, 재는 숫자는 플레이 수가 아니라 **카드 저장 비율**이다.
그게 안 일어나면 전제가 틀린 것이고, 걸음을 더 다듬어도 소용없다.

### 숫자

게임 23 → **24**, 노트 9 → **10**, 색인 URL 38 → **40**. 테스트 251 통과. 모바일 점검 24/24.

...
import { diff, stddev } from '../tools/framediff.js';
...
test('단색 화면은 표준편차가 0에 가깝다 (빈 캔버스 판정 기준)', async () => {
  const frame = await solid(40, 40, 40);
  assert.ok((await stddev(frame)) < 1);
});

test('절반씩 검정/흰색이면 표준편차가 뚜렷하게 크다 (사니티 하한선)', async () => {
  const half = await sharp({ create: { width: W, height: H, channels: 3, background: { r: 0, g: 0, b: 0 } } })
    ...
    .png()
    .toBuffer();
  assert.ok((await stddev(half)) > 50);
});

test('어두운 배경에 흩어진 작은 네온 스프라이트는 표준편차가 빈 캔버스 임계값(3)을 넘는다', async () => {
  // "빈 캔버스"로 오판할 위험이 가장 큰 경우: 화면 대부분이 어둡고, 밝은 부분은
  // 작고 성글게 흩어져 있을 때다(64x48로 축소하면서 평균화되어 표준편차가 뭉개질 수 있음).
  ...
  const frame = await sharp({ create: { width: W, height: H, channels: 3, background: { r: 10, g: 10, b: 18 } } })
    ...
    .png()
    .toBuffer();
  assert.ok((await stddev(frame)) >= 3);
});
